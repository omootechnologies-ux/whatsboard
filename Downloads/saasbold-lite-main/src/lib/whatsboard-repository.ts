...
function computeFollowUpStatus(record: FollowUpRecord): FollowUpRecord["status"] {
...
function computeDashboardStats() {
  ...
  const activeOrders = state.orders.filter((order) => order.stage !== "delivered").length;
  const overdueFollowUps = state.followUps.filter((followUp) => computeFollowUpStatus(followUp) === "overdue").length;
  ...
  const delivered = state.orders.filter((order) => order.stage === "delivered").length;
  const conversionRate = state.orders.length ? Math.round((delivered / state.orders.length) * 100) : 0;
...
export function listOrders(query: OrdersQuery = {}) {
  ...
  return state.orders
    ...
    .filter((order) => {
      ...
      const haystack = [order.id, order.customerName, order.channel, order.deliveryArea, order.items.join(" ")].join(" ");
...
export function listCustomers(query: CustomersQuery = {}) {
  ...
  return state.customers
    .filter((customer) => (query.status ? customer.status === query.status : true))
    .filter((customer) => {
      if (!search) return true;
...
export function listFollowUps(query: FollowUpsQuery = {}) {
  ...
  return state.followUps
    ...
    .filter((followUp) => (query.status ? followUp.status === query.status : true))
    .filter((followUp) => {
      if (!search) return true;
      const haystack = [followUp.title, followUp.customerName, followUp.note, followUp.orderId || ""].join(" ");
      return includesText(haystack, search);
    })
...
export function listPayments(query: PaymentsQuery = {}) {
  ...
  return state.payments
    ...
    .filter((payment) => (query.method ? payment.method === query.method : true))
    .filter((payment) => {
      if (!search) return true;
      const haystack = [payment.orderId, payment.customerName, payment.reference, payment.method].join(" ");
      return includesText(haystack, search);
    })
...
export function getDashboardSnapshot() {
  ...
  return {
    ...
    followUps: state.followUps.map((item) => ({ ...item, status: computeFollowUpStatus(item) })),
  };
}
