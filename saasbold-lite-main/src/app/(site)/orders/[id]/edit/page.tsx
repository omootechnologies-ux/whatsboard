  ...
  return (
    <div className="space-y-5 lg:space-y-6">
      <PageHeader
        ...
        primaryAction={
          <button className="wb-button-primary" type="submit" form="edit-order-form">
      ...
      <SectionCard title="Edit order fields" description="Production-ready structure for future backend patch endpoints.">
        <form id="edit-order-form" action={`/api/orders/${order.id}`} method="post" className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className="mb-2 block text-sm font-semibold text-[var(--color-wb-text)]">Customer name</label>
            <input name="customerName" required className="wb-input" defaultValue={order.customerName} />
          </div>
          <div>
            <label className="mb-2 block text-sm font-semibold text-[var(--color-wb-text)]">Stage</label>
            <select name="stage" className="wb-input" defaultValue={order.stage}>
          ...
          <div>
            ...
            <select name="paymentStatus" className="wb-input" defaultValue={order.paymentStatus}>
          ...
          <div>
            ...
            <input name="amount" required type="number" min="1" className="wb-input" defaultValue={String(order.amount)} />
          </div>
          <div className="sm:col-span-2">
            <label className="mb-2 block text-sm font-semibold text-[var(--color-wb-text)]">Notes</label>
            <textarea name="notes" className="min-h-[120px] w-full rounded-2xl border border-[var(--color-wb-border)] bg-white px-4 py-3 text-sm text-[var(--color-wb-text)]" defaultValue={order.notes} />
          </div>
        </form>
      </SectionCard>
    </div>
