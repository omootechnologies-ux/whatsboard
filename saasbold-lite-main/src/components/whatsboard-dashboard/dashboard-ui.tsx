...
import type { CustomerRecord, FollowUpRecord, OrderRecord, PaymentStatus } from "@/data/whatsboard";
...
export function stageTone(stage: OrderRecord["stage"]) {
  ...
  if (stage === "paid" || stage === "packing" || stage === "dispatched") return "primary";
...
export function DashboardShellFrame({
  ...
  const currentLabel =
    desktopNav.find((item) => (item.href === "/" ? pathname === "/" : pathname.startsWith(item.href)))?.label ?? "Dashboard";
            ...
            <div className={`flex items-center gap-3 ${collapsed ? "mx-auto" : ""}`}>
              ...
              {!collapsed ? (
                <div>
                  ...
                  <p className="text-sm font-black tracking-[-0.03em] text-[var(--color-wb-text)]">Seller control room</p>
                </div>
              ) : null}
            </div>
            {!collapsed ? (
            ...
            <div className="px-4 pt-4">
              <div className="wb-soft-card p-4">
                ...
                <p className="mt-3 text-sm leading-6 text-[var(--color-wb-text-muted)]">
                  Turn WhatsApp, Instagram, TikTok, and Facebook orders into one clean daily workflow.
                </p>
              </div>
            </div>
          )}
                ...
                <section key={section.group}>
                  ...
                  <div className={`space-y-2 ${collapsed ? "" : "mt-2"}`}>
                    {section.items.map((item) => (
                      <AppLink key={item.href} item={item} pathname={pathname} collapsed={collapsed} />
                    ))}
                  </div>
                </section>
              ))}
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <header className="sticky top-0 z-30 border-b border-[var(--color-wb-border)] bg-[var(--color-wb-background)]/92 backdrop-blur-xl">
            <div className="mx-auto flex w-full max-w-[1600px] items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:px-8">
              ...
              <label className="relative hidden max-w-md flex-1 md:block">
                ...
                <input className="wb-input pl-11" placeholder="Search orders, customers, payments..." />
              </label>
          ...
          {mobileOpen ? (
            <div className="fixed inset-0 z-40 lg:hidden">
              <button className="absolute inset-0 bg-black/30" onClick={() => setMobileOpen(false)} />
              <div className="absolute left-0 top-0 h-full w-[84vw] max-w-[22rem] border-r border-[var(--color-wb-border)] bg-white p-4 shadow-[0_30px_60px_rgba(17,17,17,0.18)]">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-lg font-black tracking-[-0.03em] text-[var(--color-wb-text)]">WhatsBoard</p>
                    <p className="text-sm text-[var(--color-wb-text-muted)]">Seller control room</p>
                  </div>
                  <button
                  ...
                  {groupedNav.map((section) => (
                    <section key={section.group}>
                      ...
                      <div className="mt-2 space-y-2">
                        {section.items.map((item) => (
                          <AppLink key={item.href} item={item} pathname={pathname} onClick={() => setMobileOpen(false)} />
                        ))}
                      </div>
                    </section>
                  ))}
                ...
                return (
                  <Link
                    ...
                    className={`flex min-w-0 flex-1 flex-col items-center justify-center gap-1 px-2 py-2 text-[10px] font-semibold ${
                      active ? "text-[var(--color-wb-primary)]" : "text-[var(--color-wb-text-muted)]"
                    }`}
                  >
                    <span
  ...
  return (
    <div className="wb-shell-card p-5 sm:p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="min-w-0">
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[var(--color-wb-primary)]">WhatsBoard</p>
...
export function EmptyState({
  ...
  return (
    <div className="rounded-[22px] border border-dashed border-[var(--color-wb-border-strong)] bg-[var(--color-wb-surface-alt)] px-5 py-8 text-center">
      <p className="text-base font-semibold text-[var(--color-wb-text)]">{title}</p>
      <p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-[var(--color-wb-text-muted)]">{detail}</p>
  ...
  return (
    <div className="overflow-x-auto rounded-[22px] border border-[var(--color-wb-border)] bg-white">
      <table className="min-w-full divide-y divide-[var(--color-wb-border)]">
        ...
        <tbody className="divide-y divide-[var(--color-wb-border)]">{children}</tbody>
      </table>
    </div>
  );
}
...
export function DataRow({
  ...
  return <tr className="align-top transition hover:bg-[var(--color-wb-surface-alt)]/60">{children}</tr>;
}
...
export function DataCell({
  ...
  return <td className={`${compact ? "px-4 py-3" : "px-4 py-4"} text-sm text-[var(--color-wb-text)]`}>{children}</td>;
}
      ...
      {items.map((item) => {
        ...
        return (
          <div key={item.label} className="wb-soft-card p-4">
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[var(--color-wb-text-muted)]">{item.label}</p>
            <p className={`mt-2 text-2xl font-black tracking-[-0.04em] ${toneClass}`}>{item.value}</p>
          </div>
        );
      })}
    </div>
  ...
  return (
    <div className="wb-shell-card p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--color-wb-text-muted)]">{label}</p>
          <p className="mt-3 text-3xl font-black tracking-[-0.04em] text-[var(--color-wb-text)]">{value}</p>
          <p className="mt-2 text-sm leading-6 text-[var(--color-wb-text-muted)]">{detail}</p>
        </div>
        {accent ? (
  ...
  return (
    <section className="wb-shell-card p-5 sm:p-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h3 className="text-xl font-black tracking-[-0.03em] text-[var(--color-wb-text)]">{title}</h3>
          {description ? <p className="mt-2 text-sm leading-6 text-[var(--color-wb-text-muted)]">{description}</p> : null}
        </div>
        {actions}
      </div>
        ...
        {chips.length ? (
          <div className="flex flex-wrap gap-2">
            {chips.map((chip) => {
              ...
              return (
                <Link key={`${chip.key}-${chip.label}`} href={buildHref({ [chip.key]: chip.value ?? null })}>
...
export function PaymentBadge({ status }: { status: PaymentStatus }) {
  ...
  return <span className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold capitalize ${tones[tone]}`}>{status}</span>;
}
...
export function StageBadge({ stage }: { stage: OrderRecord["stage"] }) {
  ...
  const tones = {
    success: "bg-emerald-50 text-emerald-700 border-emerald-100",
    warning: "bg-amber-50 text-amber-700 border-amber-100",
    primary: "bg-[var(--color-wb-primary-soft)] text-[var(--color-wb-primary)] border-[var(--color-wb-border)]",
    neutral: "bg-slate-50 text-slate-700 border-slate-100",
  } as const;
  ...
  return (
    <div className="rounded-[24px] border border-[var(--color-wb-border)] bg-white p-4 shadow-[0_14px_28px_rgba(17,17,17,0.04)]">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="truncate font-semibold text-[var(--color-wb-text)]">{order.customerName}</p>
          <p className="mt-1 text-xs text-[var(--color-wb-text-muted)]">{order.channel} • {order.deliveryArea}</p>
        </div>
        <StageBadge stage={order.stage} />
      </div>
      <div className="mt-4">
        <p className="text-sm font-medium text-[var(--color-wb-text)]">{order.items[0]}</p>
        <p className="mt-1 text-sm text-[var(--color-wb-text-muted)]">{order.items.length > 1 ? `${order.items.length} items total` : "1 item total"}</p>
      </div>
      <div className="mt-4 flex items-center justify-between gap-3">
        <div>
          <p className="text-lg font-black tracking-[-0.03em] text-[var(--color-wb-primary)]">{formatCurrency(order.amount)}</p>
          <p className="mt-1 text-xs text-[var(--color-wb-text-muted)]">Updated {formatDate(order.updatedAt)}</p>
        </div>
    ...
    <div className="rounded-[24px] border border-[var(--color-wb-border)] bg-[var(--color-wb-surface-alt)] p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-[var(--color-wb-text-muted)]">{orders.length} orders</p>
        </div>
      </div>
      ...
      {items.map((item, index) => (
        <div key={`${item.title}-${index}`} className="flex gap-4">
          <div className="flex flex-col items-center">
            ...
            {index !== items.length - 1 ? <span className="mt-2 h-full w-px bg-[var(--color-wb-border)]" /> : null}
          </div>
          <div className="min-w-0 flex-1 rounded-[22px] border border-[var(--color-wb-border)] bg-[var(--color-wb-surface-alt)] p-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              {item.meta ? <span className="text-xs text-[var(--color-wb-text-muted)]">{item.meta}</span> : null}
            </div>
            <p className="mt-2 text-sm leading-6 text-[var(--color-wb-text-muted)]">{item.detail}</p>
          </div>
        </div>
      ))}
    </div>
  ...
  return (
    <SectionCard title={title} description={description}>
      <div className="flex h-64 items-end gap-3 rounded-[24px] border border-[var(--color-wb-border)] bg-[var(--color-wb-surface-alt)] p-4">
        {data.map((item) => (
          <div key={String(item.label)} className="flex min-w-0 flex-1 flex-col items-center gap-3">
            <div className="relative flex h-full w-full items-end justify-center">
              <div
                className="w-full max-w-[52px] rounded-t-[18px] bg-gradient-to-b from-[#1f7c5d] to-[var(--color-wb-primary)]"
                style={{ height: `${Math.max((Number(item[dataKey]) / max) * 100, 10)}%` }}
              />
            </div>
            <div className="text-center">
              <p className="text-sm font-semibold text-[var(--color-wb-text)]">{Number(item[dataKey]).toLocaleString()}</p>
              <p className="mt-1 text-xs uppercase tracking-[0.14em] text-[var(--color-wb-text-muted)]">{String(item.label)}</p>
            </div>
          </div>
        ))}
      </div>
...
export function CustomerRow({ customer }: { customer: CustomerRecord }) {
  return (
    <div className="flex flex-col gap-3 rounded-[22px] border border-[var(--color-wb-border)] bg-[var(--color-wb-surface-alt)] p-4 sm:flex-row sm:items-center sm:justify-between">
      <div className="min-w-0">
        <p className="font-semibold text-[var(--color-wb-text)]">{customer.name}</p>
        <p className="mt-1 text-sm text-[var(--color-wb-text-muted)]">{customer.phone} • {customer.location}</p>
      </div>
...
export function FollowUpCard({ item }: { item: FollowUpRecord }) {
  ...
  return (
    <div className="rounded-[24px] border border-[var(--color-wb-border)] bg-white p-4 shadow-[0_14px_28px_rgba(17,17,17,0.04)]">
      ...
      <p className="mt-4 text-sm leading-6 text-[var(--color-wb-text-muted)]">{item.note}</p>
      <div className="mt-4 flex items-center justify-between gap-3 text-sm">
        <span className="font-semibold text-[var(--color-wb-text)]">{formatDate(item.dueAt)}</span>
