...
export async function getDashboardData() {
  ...
  const avgOrderValue = normalizedOrders.length ? normalizedOrders.reduce((sum, o) => sum + o.amount, 0) / normalizedOrders.length : 0;

  return {
    orders: normalizedOrders,
    metrics: { totalOrders: normalizedOrders.length, paidOrders, unpaidOrders, unpaidValue, avgOrderValue }
  };
}
...
export async function getCustomersData() {
  ...
  return (data ?? []).map((item: any) => {
    ...
    return { id: item.id, name: item.name, phone: item.phone, area: item.area ?? "", totalSpent, orderCount, lastOrderDate, isRepeat: orderCount > 1, status: item.status };
  });
}
...
export async function getFollowUpsData() {
  ...
  const { data } = await supabase
    .from("follow_ups")
    .select("id, due_at, note, completed, orders(id, product_name, delivery_area, customers(name, phone))")
    .eq("business_id", businessId)
    .order("created_at", { ascending: false });
  ...
  return (data ?? []).map((item: any) => {
    ...
    return { id: item.id, dueAt: item.due_at, note: item.note, completed: item.completed, orderId: order?.id ?? "", product: order?.product_name ?? "", area: order?.delivery_area ?? "", customerName: customer?.name ?? "Unknown", phone: customer?.phone ?? "" };
  });
}
